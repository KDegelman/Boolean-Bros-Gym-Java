...
public class ClientHndlr implements Runnable {
    ...
    public ClientHndlr(Socket socket, Server server) {
        this.socket = socket;
        this.server = server;
        try {
        ...
        try {
            while ((command = reader.readLine()) != null) {
                ...
                switch (commandArgs[0]) {
                    case "CREATE" -> server.createEntry(Integer.parseInt(commandArgs[1]), commandArgs[2], commandArgs[3],
                                commandArgs[4], commandArgs[5], commandArgs[6], commandArgs[7], commandArgs[8], writer);
